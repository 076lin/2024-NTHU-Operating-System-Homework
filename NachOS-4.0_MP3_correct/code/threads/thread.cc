...
void Thread::Yield() {
    ...
    if (kernel->scheduler->Preempt()) {  // Check if current should be preempted
        nextThread = kernel->scheduler->FindNextToRun();
        if (nextThread != NULL) {
            updateAccuRunTime(kernel->stats->totalTicks);
...
void Thread::Sleep(bool finishing) {
    ...
    updateAccuRunTime(kernel->stats->totalTicks);
    DEBUG(dbgSche, "[D] Tick [" << kernel->stats->totalTicks
                                << "]: Thread [" << ID << "] update approximate burst time, from: ["
                                << approxBurstTime << "], add [" << accuRunTime << "], to [" << 0.5 * (accuRunTime + approxBurstTime) << "]");
    approxBurstTime = 0.5 * (accuRunTime + approxBurstTime);
    accuRunTime = 0;
