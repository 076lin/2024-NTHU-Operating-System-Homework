...
int Directory::GetDirSector(char *name){
    ...
    int index = 1;
    while(name[index]!='\0'&&name[index]!='/') index++;
    char entry[256];
    strncpy(entry,name+1,index-1);
    entry[index-1] = '\0';
    int i = FindIndex(entry);
    int sector = -1;
    if(name[index] != '\0'){
        Directory *dir = new Directory(NumDirEntries);
        OpenFile *file = new OpenFile(table[i].sector);
        dir->FetchFrom(file);
        sector = dir->GetDirSector(name+index);
        delete dir;
        delete file;
    }else sector = table[i].sector;
    DEBUG('f',"Directory::GetDirSector Name: "<<name<<" at Sector: "<<sector);
    return sector;
}
//MP4 end
//----------------------------------------------------------------------
